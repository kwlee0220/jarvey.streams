...
public final class BinaryAssociation {
	...
	public static List<List<BinaryAssociation>> splitIntoConnectedClosure(Iterable<BinaryAssociation> assocList) {
		...
		for ( BinaryAssociation ba: assocList ) {
			...
			Set<TrackletId> left = Funcs.findFirst(idClosures, c -> c.contains(ba.getLeftTrackletId())).orElse(null);
			Set<TrackletId> right = Funcs.findFirst(idClosures, c -> c.contains(ba.getRightTrackletId())).orElse(null);
