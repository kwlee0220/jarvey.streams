 ...
 */
public final class FeatureAssociationStreamBuilder2 {
	...
	public FeatureAssociationStreamBuilder2(MCMOTConfig configs, AssociationCollection associations,
		...
		KStream<String,String> trackletAssociateds
			= trackFeatures
				...
				.map((k, ba) -> {
					...
					for ( Association assoc: m_associations.add(ba) ) {
						assocStore.addAssociation(assoc);
						m_finalAssociations.add(assoc);
						if ( assoc.containsTracklet(trkId) ) {
							selected = assoc;
						}
					}
					m_statusRegistry.setAssigned(trkId, selected);
					return KeyValue.pair(trkId.toString(), selected.getId());
				});
			...
			= trackFeatures
				...
				.flatMap((k,tfeat) -> {
					...
					if ( !status.isAssociated() ) {
						...
						m_finalAssociations.removeIf(assoc -> Duration.ofMillis(tfeat.getTimestamp() - assoc.getTimestamp())
